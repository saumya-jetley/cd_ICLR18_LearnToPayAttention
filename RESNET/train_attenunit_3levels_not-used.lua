...
local net_1_1 = dofile('models/'..opt.model_1_1..'.lua'):cuda()
local net_3_1 = dofile('models/'..opt.model_3_1..'.lua'):cuda()

local net_1_2 = dofile('models/'..opt.model_1_2..'.lua'):cuda()
local net_3_2 = dofile('models/'..opt.model_3_2..'.lua'):cuda()

local net_1_3 = dofile('models/'..opt.model_1_3..'.lua'):cuda()
local net_2_3 = dofile('models/'..opt.model_2_3..'.lua'):cuda()
local net_3_3 = dofile('models/'..opt.model_3_3..'.lua'):cuda()

local net_4 = dofile('models/'..opt.model_4..'.lua'):cuda()
----------------------------------------------------------------
...
do
  ...
  ------Global, Atten and Match Models----------INITIALIZATION-----------------
   ...
   cudnn.benchmark = true
   if opt.cudnn_fastest then   
      for i,v in ipairs(net_3_1:findModules'cudnn.SpatialConvolution') do v:fastest() end
   ...
   ------
   print(net_3_1); print('Network has', #model_3_1:findModules'cudnn.SpatialConvolution', 'convolutions')
...
table.insert(model_all, model_3_1)
...
------Forward
   ...
   local att_con_160 = model_3_1:forward({lfeat_160,gfeat_640})
   local att_con_320 = model_3_2:forward({lfeat_320,gfeat_640})
   local att_con_640 = model_3_3:forward({lfeat_640,gfeat_640})

   local pred = model_4:forward({att_con_160[2],att_con_320[2],att_con_640[2]})
------Loss
   local loss = criterion:forward(pred, targets)
   local df_pred = criterion:backward(pred, targets)
------Backward
   local df_att_con_combined2 = model_4:backward({att_con_160[2],att_con_320[2],att_con_640[2]}, df_pred)

   local df_feat_640 = model_3_3:backward({lfeat_640,gfeat_640}, {torch.rand(att_con_640[1]:size()):cuda():fill(0), df_att_con_combined2[3]})
   local df_feat_320 = model_3_2:backward({lfeat_320,gfeat_640}, {torch.rand(att_con_320[1]:size()):cuda():fill(0), df_att_con_combined2[2]})
   local df_feat_160 = model_3_1:backward({lfeat_160, gfeat_640},{torch.rand(att_con_160[1]:size()):cuda():fill(0), df_att_con_combined2[1]})

   local df_lfeat_640 = model_2_3:backward(lfeat_640, (df_feat_160[2]+df_feat_320[2]+df_feat_640[2])/3)

   local df_lfeat_chain_320 = model_1_3:backward(lfeat_320, (df_lfeat_640+df_feat_640[1])/2)
   local df_lfeat_chain_160 = model_1_2:backward(lfeat_160, (df_lfeat_chain_320+df_feat_320[1])/2)
   model_1_1:backward(inputs, (df_feat_160[1]+df_lfeat_chain_160)/2)
...
------------save the models-------------------------------------
  ...
  torch.save(opt.save..'/model_3_1.t7', net_3_1:clearState())
