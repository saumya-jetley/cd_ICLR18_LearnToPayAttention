...
function train()
    ...
    for t,v in ipairs(indices) do
		...
		---------forward
                  ...
                  local df_feat_2 = matten_2:backward({lfeat_2,gfeat_3}, {torch.rand(att_con_2[1]:size()):cuda():fill(0), df_context[2]}) 
                  local df_feat_1 = matten_1:backward({lfeat_1,gfeat_3}, {torch.rand(att_con_1[1]:size()):cuda():fill(0), df_context[1]})  
            
                  local df_lfeat_3 = mglobal_3:backward(lfeat_3, (df_feat_1[2]+df_feat_2[2]+df_feat_3[2])/3)                  
                  
                  local df_lfeat_chain_2 = mlocal_3:backward(lfeat_2,(df_lfeat_3+df_feat_3[1])/2)
                  local df_lfeat_chain_1 = mlocal_2:backward(lfeat_1,(df_lfeat_chain_2+df_feat_2[1])/2)                      
                  mlocal_1:backward(inputs,(df_lfeat_chain_1+df_feat_1[1])/2)
