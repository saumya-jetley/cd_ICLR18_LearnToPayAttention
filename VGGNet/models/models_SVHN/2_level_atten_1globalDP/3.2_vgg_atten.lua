...
local gfeat_repl1 = nn.Replicate(8,2,1)(gfeat) -- spatial dimension 1 of lfeat
local gfeat_repl2 = nn.Replicate(8,2,1)(gfeat_repl1) -- spatial dimension 2 of lfeat

--local glfeat_add = nn.CAddTable()({gfeat_repl2, lfeat})
--local att_mag = nn.SpatialConvolution(512,1,1,1,1,1,0,0)(glfeat_add)
local att_mul = nn.CMulTable()({lfeat, gfeat_repl2})
local att_mag = nn.Sum(1,3)(att_mul)

local att_mag_reshaped = nn.Reshape(8*8)(att_mag)
local att_mag_smax = nn.SoftMax()(att_mag_reshaped)

local att_2d_mask = nn.Reshape(8,8)(att_mag_smax)
local att_retrans = nn.Replicate(512,2,1)(att_mag_smax)

local lfeat_reshaped = nn.Reshape(512, 64)(lfeat)
